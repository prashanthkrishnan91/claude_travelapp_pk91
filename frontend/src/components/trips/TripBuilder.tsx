...
export function TripBuilder({ tripId, destination, startDate, endDate, initialDays, initialResults }: TripBuilderProps) {
  ...
  // Auto-generate itinerary days from trip date range.
  // Runs once on mount; skips only when the DB already has the full expected set of days.
  // Guards against double-fire in StrictMode via autoGeneratedRef.
  useEffect(() => {
    if (autoGeneratedRef.current || !startDate || !endDate) return;
    const start = new Date(startDate + "T00:00:00");
    const end   = new Date(endDate   + "T00:00:00");
    const expectedCount = Math.round((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
    if (expectedCount <= 0 || expectedCount > 60) return;
    // All days already present — nothing to generate
    if (days.length >= expectedCount) return;
    autoGeneratedRef.current = true;
    (async () => {
      // Only create days that don't already exist (by dayNumber)
      const existingNums = new Set(days.map((d) => d.dayNumber));
      ...
      for (let i = 0; i < expectedCount; i++) {
        ...
        try {
          const day = await createDay(tripId, { dayNumber: i + 1, title: `Day ${i + 1}`, date: dateStr });
          created.push(day);
  ...
  // Whether days are date-locked (derived from trip dates) — prevents manual Add Day
