...
export function TripBuilder({ tripId, destination, startDate, endDate, initialDays, initialResults }: TripBuilderProps) {
  ...
  // Auto-generate itinerary days from trip date range when no days exist yet.
  // Runs once on mount; guarded by autoGeneratedRef to avoid double-fire in StrictMode.
  useEffect(() => {
    if (autoGeneratedRef.current || !startDate || !endDate || days.length > 0) return;
    autoGeneratedRef.current = true;
    const start = new Date(startDate + "T00:00:00");
    const end   = new Date(endDate   + "T00:00:00");
    const dayCount = Math.round((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
    if (dayCount <= 0 || dayCount > 60) return;
    (async () => {
