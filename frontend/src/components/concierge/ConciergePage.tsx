...
// ─── Closed-signal helpers (same logic as AIConciergePanel) ──────────────────
...
type ClosedSignalSource = Partial<{
  name: unknown; title: unknown; summary: unknown; description: unknown;
  reason: unknown; source: unknown; sourceText: unknown; rawText: unknown;
  url: unknown; sourceUrl: unknown; snippet: unknown;
}>;

function hasClosedSignal(card: ClosedSignalSource): boolean {
  const textBlob = [
    card.name, card.title, card.summary, card.description,
    card.reason, card.source, card.sourceText, card.rawText,
    card.url, card.sourceUrl, card.snippet,
  ].map((v) => String(v ?? "").toLowerCase()).join("\n");
  return CLOSED_SIGNAL_PATTERNS.some((signal) => textBlob.includes(signal));
}

type OperationalBadgeCard = {
  googleVerification?: {
...
// ─── Card meta composer (same logic as AIConciergePanel) ─────────────────────
...
function pickCardMeta(card: DisplayCard): string[] {
  ...
  if (ratingBase) {
    ...
    if (addrTrimmed && ratingBase.includes(addrTrimmed)) {
      const stripped = ratingBase
        .slice(0, ratingBase.indexOf(addrTrimmed))
        .replace(/\s*·\s*$/, "")
        .trim();
      if (stripped) stem = stripped;
    }
    const parts: string[] = [stem];
    if (price && !/\$|Free\b|EUR/.test(stem)) parts.push(price);
