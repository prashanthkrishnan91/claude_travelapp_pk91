...
def list_days(trip_id: UUID, user_id: CurrentUserID, db: DB) -> List[ItineraryDay]:
    """Return all itinerary days for a trip, ordered by day_number."""
    return ItineraryService(db).list_days(trip_id, user_id)
...
def create_day(trip_id: UUID, payload: ItineraryDayCreate, user_id: CurrentUserID, db: DB) -> ItineraryDay:
    """Add an itinerary day to a trip."""
    return ItineraryService(db).create_day(payload, user_id)


@router.get("/{trip_id}/days/{day_id}", response_model=ItineraryDay)
def get_day(trip_id: UUID, day_id: UUID, user_id: CurrentUserID, db: DB) -> ItineraryDay:
    """Fetch a single itinerary day."""
    return ItineraryService(db).get_day(day_id, user_id)


@router.patch("/{trip_id}/days/{day_id}", response_model=ItineraryDay)
def update_day(
    trip_id: UUID, day_id: UUID, payload: ItineraryDayUpdate, user_id: CurrentUserID, db: DB
) -> ItineraryDay:
    """Partially update an itinerary day."""
    return ItineraryService(db).update_day(day_id, payload, user_id)


@router.delete(
    "/{trip_id}/days/{day_id}", status_code=status.HTTP_204_NO_CONTENT
)
def delete_day(trip_id: UUID, day_id: UUID, user_id: CurrentUserID, db: DB) -> None:
    """Delete an itinerary day and all its items."""
    ItineraryService(db).delete_day(day_id, user_id)
...
def list_items(trip_id: UUID, day_id: UUID, user_id: CurrentUserID, db: DB) -> List[ItineraryItem]:
    """Return all items for a day, ordered by position."""
    return ItineraryService(db).list_items(day_id, user_id)
...
def create_item(
    trip_id: UUID, day_id: UUID, payload: ItineraryItemCreate, user_id: CurrentUserID, db: DB
) -> ItineraryItem:
    """Add an item (flight, hotel, activity…) to an itinerary day."""
    return ItineraryService(db).create_item(payload, user_id)
...
def create_trip_item(payload: ItineraryItemDirectCreate, user_id: CurrentUserID, db: DB) -> ItineraryItem:
    """Add a trip-level item (e.g. a saved flight) without requiring a specific day."""
    return ItineraryService(db).create_trip_item(payload, user_id)


@router.get("/items/{item_id}", response_model=ItineraryItem)
def get_item(item_id: UUID, user_id: CurrentUserID, db: DB) -> ItineraryItem:
    """Fetch a single itinerary item."""
    return ItineraryService(db).get_item(item_id, user_id)


@router.patch("/items/{item_id}", response_model=ItineraryItem)
def update_item(
    item_id: UUID, payload: ItineraryItemUpdate, user_id: CurrentUserID, db: DB
) -> ItineraryItem:
    """Partially update an itinerary item."""
    return ItineraryService(db).update_item(item_id, payload, user_id)


@router.delete("/items/{item_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_item(item_id: UUID, user_id: CurrentUserID, db: DB) -> None:
    """Remove an itinerary item."""
    ItineraryService(db).delete_item(item_id, user_id)
