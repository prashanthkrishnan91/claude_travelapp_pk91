...
| **Trip Readiness (`TripReadinessCockpit`)** | collapsed disclosure | **Removed from page; file deleted** | Decision 3 — duplicate of Brief + Dayboard; mobile noise. Unmounted in PR 1; file deleted in cleanup PR |
...
4. REMOVE the TripReadinessCockpit block (page.tsx ~585–611) and its `cockpitOpen` state, the import, and the "Trip readiness · concierge notes" toggle. Do NOT add a replacement notes artifact (no durable notes data source — would fabricate content). `TripReadinessCockpit.tsx` was subsequently deleted in a dead-code cleanup PR.
