...
> **Status update (2026-05-08, post-merge of v1D internal mock cleanup):** The internal dependency on `_mock_attractions` is now eliminated. `POST /plan/day` no longer emits mock-derived attraction rows (no-cluster path returns `attractions=[]`; cluster-driven path unchanged). The four `app/services/concierge.py` callsites that fell back to `search_attractions` (ATTRACTION/PLAN_DAY, BEST_AREA/AREA_ADVICE, GENERAL_DESTINATION, `_load_context`) now fail closed or use canonical seams. `_mock_attractions`, `SearchService.search_attractions`, `_compute_attraction_ai_score`, and `_compute_attraction_tags` are deleted. The `attractions` cache namespace is removed from `_LEGACY_MOCK_DEPENDENT_NAMESPACES`. `LEGACY_PRODUCT_MOCK_FUNCTIONS` is now `{flights, hotels, restaurants}`. Strict reintroduction guard added to `backend/tests/test_product_surface_pruning_v1a.py`.
