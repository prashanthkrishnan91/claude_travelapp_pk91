...
**What was built**:
- Backend: `create_trip_with_search` fails closed with `HTTP 503 {code: "provider_unavailable", message: ...}` when `flights`, `hotels`, and `round_trip_pairs` are all empty. No trip, no itinerary days, no itinerary items are created on that branch. Existing 422 invalid-destination path preserved.
