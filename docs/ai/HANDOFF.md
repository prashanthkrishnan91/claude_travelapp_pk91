...
Last updated: 2026-05-22 (Saved note carryover into Trip Ideas; current branch)
...
**Stage 3.5 — design adoption across the Atelier rooms.** Stage 3 exit completed earlier (2026-05-14). The outside-trip Concierge (`/concierge`) is the dark Private Travel Salon. The outside-trip Explore (`/explore`) is the dark **Observatory**. Saved (`/saved`) is now the **Private Folio** — the deliberately *light* paper room (the third sibling). Trip detail (`/trips/[id]`) is the **Journey Desk** — v1A #467 (cover + Brief + Dayboard); v1B #468 (Ideas Tray + Notes); v1C #469 (Expanded Day + Decision Strip); v1D #470 (consolidation + polish) — **v1 complete**. **v2A (Map Fold-Out, Trip Lens) is the current branch**; Day/Idea lenses are v2B.

### Journey Desk v2A — Map Fold-Out, Trip Lens (current branch)
...
### Next step
Journey Desk **v2A** (Map Fold-Out, Trip Lens) is the current branch. Next: **v2B** — Day Lens / Idea Lens, and **real pin plotting** once the **map-coordinate contract** lands (persist real `lat/lng` per placed item on `ItineraryItem.details` at add-time; expose for plotting). Without that contract, keep the honest map-ready list — do not fabricate positions (the discovery `TripMapView` `goldenSpread` fallback is exactly what to avoid). Other deferred polish: full desktop three-zone, fold/remove the standalone legacy Ideas tab once parity holds, consolidate the day-part classifier (mirrored in `lib/dayParts.ts` + `ItineraryDayColumn`). Placement stays day-level (`assignIdeaToDay`); no slot-level persistence.
