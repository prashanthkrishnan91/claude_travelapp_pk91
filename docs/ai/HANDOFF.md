...
- 2026-05-15 — **Outside-trip Concierge Stabilization (Level 2 backend+frontend).** Root cause 1: `INTENT_ATTRACTIONS` was absent from `_FAST_DYNAMIC_INTENTS` in `concierge.py` → "top attractions" / "museums" fell into legacy Tavily/live_research path (~123s). Fix: added `INTENT_ATTRACTIONS` to `_FAST_DYNAMIC_INTENTS`; attraction queries now use `semantic_retrieval_v1` fast Google-verified card path. Root cause 2: `ConciergePage.tsx` silently hid user messages (`return null`), had no localStorage persistence, and had no clear-chat control. Fix: user messages render as right-aligned bubbles; `useEffect` loads/saves `concierge_outside_trip_transcript_v1` key to localStorage; Trash2 clear-chat button clears state + localStorage. No SQL migration. No new providers. No design overhaul. 22 backend tests in `test_concierge_attraction_fast_path.py`; 15 frontend tests in `concierge-transcript-persistence.test.mjs`. Wrong-vertical guard (food/bar only) unchanged — confirmed not applied to attractions/museums/hotels.
