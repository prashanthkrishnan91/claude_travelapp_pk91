...
- 2026-05-12 — **Provider Registry v1 + Explore Provider Scope Reset.** Added `backend/app/services/provider_registry.py` as the central provider policy (ProviderRole enum, ProviderEntry dataclass, PROVIDER_REGISTRY dict). Approved stack: Google Places (canonical/addable cards), Anthropic (reasoning only), Tavily (research only), Yelp (enrichment only), OpenWeather (weather only). Disabled/quarantined: Duffel Flights, Duffel Stays, Amadeus, Brave, Serper, Foursquare. Surgical refactors: `live_research.select_default_provider()` checks registry before activating Brave/Serper; `flights_provider.get_flight_provider()` checks registry before activating Duffel. 38 targeted tests in `test_provider_registry_v1.py`. No SQL. No UI change. No Concierge notes change. Duffel/Amadeus are no longer active roadmap items.
