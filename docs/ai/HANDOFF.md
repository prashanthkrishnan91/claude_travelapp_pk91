...
- **Tests:** `tests/atmospheric-background-system-v1.test.mjs` 24/24;
  `tests/atmospheric-boutique-art-direction-8n.test.mjs` 50/50 (8N preserved).
...
`TripReadinessCockpit.tsx` was unmounted from the page in Journey Desk PR 1 (#496) and confirmed by repo-wide grep to have zero active imports in `frontend/src/**/*.tsx`. Deleted the file and its dedicated test file (`trip-readiness-cockpit.test.mjs`). Removed cockpit sections from `core-paper-planning-slice2.test.mjs` and `trip-chapter-overhaul.test.mjs`; removed entry from `package.json` test list. Updated blueprint doc and HANDOFF to reflect deletion. **IdeasTray not touched — it remains active as a quick Add-from-Ideas placement overlay; Ideas tab remains canonical idea management.** Frontend-only; no SQL, backend, provider, or behavior change. No active Journey Desk surface touched.
...
**Root cause (two gaps):** (1) `PlannedAttraction`/`PlannedRestaurant` (and `AttractionResult`) carried **no `place_id`/`google_maps_uri`**, so even when the provider returned identity, `/plan/day` dropped canonical place identity. (2) When the bulk Google Places text search returned a place without `location`, `lat/lng` were `None` and nothing recovered them.

**Fix (resolve before persist, reuse existing provider):**
- `backend/app/models/plan.py`: `PlannedAttraction`/`PlannedRestaurant` gain optional `place_id` + `google_maps_uri` (default `None`, never fabricated).
- `backend/app/models/search.py`: `AttractionResult` gains `place_id`/`provider_place_id`/`google_maps_uri` (parity with `RestaurantResult`).
- `backend/app/services/search.py`: `search_attraction_results` now stamps the real `place_id`/`google_maps_uri`; new `resolve_place_details(place_id)` does a Google Places **Place Details GET** on the **same provider/endpoint family + same `GOOGLE_PLACES_API_KEY`** (no new provider architecture), returning `{lat,lng,google_maps_uri,address}` only on a real location — fails closed to `None` (no key/httpx/empty/error), never geocodes.
- `backend/app/routes/plan.py`: forwards `place_id`/`google_maps_uri` on both the canonical-search and cluster paths; new `_resolve_day_plan_coords(response, search)` runs before return — for each planned item that is place-like (has a Google `place_id`, incl. `gp-`-prefixed `id`) but missing `lat/lng`, it resolves via `resolve_place_details` and fills `lat/lng` (+ `google_maps_uri`). Items that already have coords are **not** re-resolved (no extra provider call); resolution only fires for the genuine gap. On failure the item keeps the honest coordinate-less fallback (display address/category/rating preserved).
- Frontend: `AttractionSearchResult` gains `placeId`/`googleMapsUri`/`providerPlaceId` (parity with `RestaurantSearchResult`); `addAttractionToDay`/`addRestaurantToDay` persist `place_id`/`provider_place_id`/`google_maps_uri` (`?? null`, never fabricated). Plan My Day handlers already route through `extractRouteableTripItemMetadata` (same write boundary as Build/Concierge), so a resolved day-plan item persists the same canonical fields and is route-hint eligible.
...
**Trip Item Metadata Parity v1 + v1.1 preview-fix** (this branch) — v1.1 patches the v1 PR after Vercel preview showed two Build/Add-to-Day Miami items (Maurice A. Ferré Park, Phillip & Patricia Frost Museum of Science) still rendering the "Add location details to improve travel hints." fallback even though address/rating/category displayed correctly. **v1.1 root cause:** v1 patched `handleAddCandidateToItinerary`, but that handler only wires the flight/hotel candidate panels. Attractions/restaurants run through `handleAddAttractionToItinerary`/`handleAddRestaurantToItinerary` → `addAttractionToDay`/`addRestaurantToDay`, which take an `AttractionSearchResult` — a lossy intermediate shape with no `placeId`/`googleMapsUri`/`formattedAddress`. `tripCandidates.itemToAttraction`/`itemToRestaurant` only read top-level `details.lat`, so when the persisted candidate row stored coordinates under alternate keys (`latitude`/`longitude`, nested `coordinates.lat`, `geo.lat`, `location.lat`), `lat`/`lng` came back `undefined` on the `AttractionSearchResult` and `addAttractionToDay` persisted `lat: null`. **v1.1 fix:** (1) `tripItemMetadata.ts` adds `readCanonicalLat`/`readCanonicalLng` resolving `lat|latitude` and `lng|longitude|lon` from top-level or nested `coordinates|geo|location|coords|position` — pure local; never fabricates. (2) `tripCandidates.itemToAttraction`/`itemToRestaurant` + `api.fetchExploreSnapshot` read via the canonical helpers. (3) `addAttractionToDay`/`addRestaurantToDay` accept optional `additionalDetails: Record<string, unknown>` and merge fill-without-overwrite (only fills base nulls/empties; never fabricates). (4) `TripBuilder` keeps a `candidateSourceItemsRef: Map<string, ItineraryItem>` keyed by both attraction id and placeId so handlers can recover the full persisted source row even when the AttractionSearchResult id differs from the ItineraryItem id. (5) `handleAddAttractionToItinerary`/`handleAddRestaurantToItinerary` look up the source ItineraryItem and pass `extractRouteableTripItemMetadata(sourceItem.details)` as `additionalDetails`. Tests grow from 18 → 31 (all pass), `travel-time-hints` + `trip-candidates-contract` + `itinerary-coordinates` still green. **No SQL. No backend / provider / search / map / Journey Desk redesign / hotel stay-span / Brief editing / Concierge routing change.**

### (original v1) Trip Item Metadata Parity v1

v1 — fixes a real metadata-loss bug at the Build/Add-to-Day source-to-trip-item boundary so routeable place-like itinerary items get the same travel-hint-capable card UX regardless of ingress path (Concierge, Build/Add-to-Day, Ideas, Saved, Explore). **Root cause:** `TripBuilder.handleAddCandidateToItinerary` for activity/meal candidates called `createItem(tripId, dayId, { itemType, title, description, location, position })` — `createItem` ignored `details`, so `lat`, `lng`, `address`, `placeId`, `category`, `rating`, `googleMapsUri` etc. were silently dropped on placement. As a result, two Build/Add-to-Day routeable places on the same day rendered the "Add location details to improve travel hints." fallback (`missing_location` in `travelHints.ts`) instead of an actual `~N min walk/drive` hint — even though Concierge-added places with the same metadata produced real hints. **Fix:** new `src/lib/tripItemMetadata.ts` defines a canonical routeable-metadata contract (`ROUTEABLE_METADATA_KEYS`, `extractRouteableTripItemMetadata`, `hasRouteableCoordinates`) that reads camelCase first with snake_case fallback and **never fabricates** missing fields. `createItem` in `src/lib/api.ts` now accepts an optional `details?: Record<string, unknown>` parameter and merges it with `bookingOptions` on the write payload (no overwrite). `TripBuilder.handleAddCandidateToItinerary` calls `extractRouteableTripItemMetadata(item.details)` and forwards the canonical blob into `createItem` for activity/meal candidates. Hotel and flight branches already had their own metadata-aware paths (`addHotelToDay` / `addOneWayFlightToDay`) and are untouched. **Ingress paths audited:** Concierge (`addStructuredConciergeItemToTrip` → already attaches `normalizeGoogleVerificationDetails` = lat/lng/placeId/maps URI) ✓; Build/Add-to-Day (`handleAddCandidateToItinerary` → **fixed**) ✓; Ideas placement (`assignIdeaToDay` → PATCH `day_id` only, never touches details, so routeable metadata is preserved as-is) ✓; Saved→trip (`addSavedItemToTrip` → already persists `lat`/`lng` via `extractItineraryCoordinates(snap)` when present, never geocoded) ✓. **Before/after:** before this PR, two Build-added routeable places on the same day produced `missing_location` hints because their persisted details rows had no lat/lng; after, the canonical extractor forwards lat/lng (and address/category/rating/maps link) into the placed item, so `computeAdjacentHints` produces a real `travel_ok` or `far_apart` hint identical to the Concierge-added pair. Honest fallback is preserved for items genuinely missing lat/lng. **No SQL.** **No new provider/search behavior.** **No Journey Desk redesign / hotel stay-span / Brief editing / Concierge routing change.** New `tests/trip-item-metadata-parity-v1.test.mjs` (18 tests, all pass) — covers extractor exports, snake-case fallbacks, no-fabrication invariants, `createItem` details forwarding, the candidate-add wiring, the Concierge/Ideas/Saved path regression guards, and the honest-fallback copy. Existing `tests/travel-time-hints.test.mjs` still passes unchanged. Test tier: Tier 1 (contract + targeted regression).

### (previous) Journey Desk Ideas tab staging-table polish

**Journey Desk Ideas tab staging-table polish** (merged #498) — polishes `TripIdeasPanel` so the Ideas tab feels like an intentional Journey Desk "staging table" for reviewing, prioritizing, noting, and placing unplaced ideas. Changes: (1) **Type-specific left accent border** on idea cards (`.jd-idea-card` + `--activity/meal/hotel/flight` modifiers) — marine for attractions, ember-brass for restaurants, sage for hotels, caution-amber for flights; immediately communicates vertical type at a glance. (2) **Recessed linen placement zone** (`.jd-idea-place-zone`) with a hairline top border and negative-margin edge-bleed — Add to Day CTA now reads as an intentional docking action, not a form afterthought; the CTA itself uses marine-ink primary styling (`bg-ds-marine-ink text-ds-paper`). (3) **Note preview** (`.jd-idea-note-preview`) — when a note is saved and the editor is closed, a 2-line italic excerpt with a brass left hairline appears below the priority row ("private marginalia" tone, mirrors IdeasTray note display). (4) **Priority / note controls separated** — "Priority" label + note toggle now in a clear row with a hairline section separator from the card identity block. (5) **Group section headers** with a hairline top rule between groups (via `groupIdx > 0` conditional) for cleaner vertical scanability. **Frontend-only; no SQL/backend/provider/search/map/hotel-stay-span/Brief-editing change. No IdeasTray change. No behavior/data-model change.** All testids, aria-pressed, handlers preserved. New `tests/journey-desk-ideas-staging-table.test.mjs` (28 tests, all pass); all existing Ideas tab and trip-ideas tests pass (85/85). 3820 total tests; 9 pre-existing failures (unchanged from baseline — confirmed identical set: salon v2 / boutique adoption / observatory / folio foundation / raw-hex / saved folio / savedItemToTrip path / tripbuilder touch-target).

### (previous) Journey Desk PR 2 — Add-to-Day / Build add-mode presentation polish

**Journey Desk PR 2 — Add-to-Day / Build add-mode presentation polish** (merged #497) — refines how the existing Build/add flow presents inside Trip Detail after PR 1 (#496). Problem: clicking `+ Add to Day X` switched the Working Surface to Build, but the candidate verticals rendered in a fixed `lg:w-80` (320px) column stranded on the left while the wide surface sat empty. Fix (presentation-only, reuses the existing add flow): the Build panel (`trip-mobile-panel-build`) now fills the Working Surface as a **centered, comfortably-wide add canvas** (`lg:max-w-3xl lg:mx-auto` instead of `lg:w-80 lg:flex-shrink-0`); the return banner is reworked into a clear **"Add mode · Adding to Day N"** masthead with a **Done · Back to Day N** action, aligned to the canvas width (`lg:max-w-3xl lg:mx-auto`). **No new add system / no provider/search change / no itinerary-item-logic change**: the same `AddToDayDrawer → flights/stays/dining/things-to-do → Build search` flow, the same `handleAddCandidateToItinerary`/`handleAddRoundTripToItinerary` adds, and the PR 1 canonical active-day sync (`focusDayId` ↔ `onActiveDayChange`, no "Add to" dropdown) are all preserved. Build is still not a permanent desktop column (hidden unless workspace==="build"); Ideas is not the only add path; two-zone base layout (Plan Rail + Itinerary Working Surface) unchanged. Mobile: add mode stays full-width single-column (the `lg:` canvas cap does not apply), tab IA unchanged. **Frontend-only; no SQL/backend/provider/search/map/hotel-stay-span/Brief-editing change.** Tests: extended `journey-desk-shell.test.mjs` (centered add canvas + "Adding to Day N" masthead + Done/Back) and updated `mobile-first-premium-8i` (Build is no longer a fixed 320px column — full-width mobile, centered desktop canvas). All touched/affected suites pass; the only remaining failure is the PRE-EXISTING `tripbuilder-command-surface` "List/Map toggle min-h-[44px]" (identical on main via git stash). tsc/next build via CI `certify`; desktop + mobile via Vercel preview. **Deferred (still):** PR 3 = durable travel-notes artifact (needs a data source); a richer dedicated Build drawer/2-column candidate layout if desired later; legacy-tab retirement stays gated to the parity plan.

### (previous) Journey Desk PR 1 — Trip Detail immersive shell + mood + desktop plan-desk

**Journey Desk PR 1 — Trip Detail (`/trips/[id]`) immersive shell + mood + desktop plan-desk** (merged #496) — implements the merged blueprint `docs/ai/design/JOURNEY_DESK_PAGE_DIRECTION_AND_BLUEPRINT.md` (PR #495) + prototype `docs/ai/concepts/journey-desk-prototype-v1.html`. **Trip Detail is now the fifth immersive room** ("The Journey Desk"): `AppShell` adds `isTripDetailRoute` (`pathname.startsWith("/trips/") && !== "/trips"`) to `isImmersiveRoom`, suppresses the SaaS sidebar via `data-atelier-shell="journey-desk"`, renders the floating `AtelierNavArtifact`, and uses the edge-to-edge `home-edge-bleed` wrapper — so `/trips/[id]` reads as a wide planning desk, not a narrow centered card on the legacy padded shell. New CSS (`@layer components`): `.journey-desk-room-canvas` (warm desk, **marine-cool** top ambient distinct from the Reading Room's sandstone), `.journey-desk-stage` (WIDE `max-width:94rem` floating paper folio), `.journey-desk-cover-band` (cover flush to stage top), `.journey-desk-layout` (desktop grid `21rem minmax(0,1fr)`), `.journey-desk-plan-rail` (sticky, recessed linen, hairline seam), `.journey-desk-surface`. `page.tsx` composes: full-width **cover band** → mobile tab switcher (`lg:hidden`) → two-zone **planning desk** = sticky **Plan Rail** (read-only `TripBrief` + `Dayboard` day spine) | wide **Working Surface** (`TripBuilder`). The old `lg:max-w-4xl` cap is removed. **The legacy `TripReadinessCockpit` "Trip readiness · concierge notes" disclosure is removed from the page** (component file subsequently deleted in dead-code cleanup; readiness actions remain on the cover: Concierge/Optimize/Edit). **No new travel-notes artifact** added (no durable notes data source — would fabricate content). Functional ownership unchanged: Brief = read-only · Itinerary = editing/day placement · Ideas tab = canonical management · IdeasTray = quick placement · Build = existing internal add/search utility (NOT converted to a drawer — PR 2). Mobile keeps the 3-tab IA, drops the readiness clutter, no duplicate surfaces. **Frontend-only; no SQL/backend/provider/search/map/itinerary/hotel-stay-span/Brief-editing change.** Tests: new `tests/journey-desk-shell.test.mjs` added to the explicit list; updated `journey-desk-v1`, `journey-desk-v1d`, `trip-chapter-overhaul`, `trip-readiness-cockpit`, `trip-detail-mobile-workspace-8k`, `boutique-visual-composition-8nc` to the new composition and corrected stale assertions from #478 (Build is not a mobile tab) + #481 (Brief review → Ideas tab). All touched/affected suites pass locally; remaining repo failures are PRE-EXISTING raw-hex / folio-palette lint failures (verified identical on main via git stash). tsc/next build via CI `certify`; desktop + mobile via Vercel preview. **Deferred (sequenced):** PR 2 = Build → right-docked utility drawer; PR 3 = durable travel-notes artifact (needs a data source); legacy-tab retirement stays gated to the parity plan.

### (superseded) My Journeys — "The Reading Room"

**My Journeys — "The Reading Room"** (on #492 branch, PR #494) — `/trips` now **adopts the existing immersive page-room shell** used by Home/Concierge/Explore/Saved: AppShell `isMyTripsRoute` joins `isImmersiveRoom`, SaaS sidebar CSS-suppressed via `data-atelier-shell="trips"`, floating `AtelierNavArtifact` nav, edge-to-edge `home-edge-bleed` canvas (no new sidebar system; nav behavior unchanged). The room sits on a full-bleed warm `trips-room-canvas` desk (radial ambient, mirrors `folio-private-desk`) with a wide **centered `max-width:78rem` floating `trips-shelf-stage`** (mirrors `folio-private-folio`; deep warm shadow + inner lining). On desktop the **"Elsewhere in the house" reference drawer is pulled above the fold** via `lg:order-2` (volumes `lg:order-3/4`); mobile stays single-column. Reading Room identity preserved: a private folio library: a floating `trips-shelf-stage` room with an editorial masthead (`folio-issue-eyebrow` "The Folio Library" + `trips-shelf-heading` "My Journeys" + real-count `roomSub` caption). The active trip is the open **current edition** — a two-zone `ContinuePlanningHero` (`trips-edition` / `trips-edition-spread` left + the single cinematic `trips-edition-plate` monogram drawn from the real destination, `lg:flex-row`). Other trips are **bound volumes** (`JourneyCard` = `folio-journey-entry` brass spine + `trips-volume-cover` + status as small-caps `trips-volume-status` text, **not** a `TripStatusBadge` pill), chaptered via `trips-chapter` editorial rules: **"On the table"** (active) and **"Bound"** (past, `trips-volume-past` quieter). Planning tools are the quiet **"Elsewhere in the house"** reference drawer (`trips-tools-shelf` + `trips-tool-panel`, all three routes preserved). Empty state is the **empty-shelf** concept (`trips-empty-plate` bound-spine motif). Mapline/captions use real data only (date range + travelers); no fabricated multi-city sequences or poetic copy. No behavior/route/data-fetch/CRUD/grouping change; one cinematic plate only; no backend/SQL/provider/map change. Governed by `docs/ai/design/JOURNEY_DESK_CLOSEOUT_AND_MY_TRIPS_VISUAL_PLAN.md` + the approved Reading Room prototype. **Next: PR 4** (TripBuilder paper-correctness pass). No legacy surface removed; legacy-tab removal stays gated to parity-plan Slice 6.
...
**What changed:**
- `ItineraryItemCard.tsx`: hotel `checkIn/checkOut` now reads `d.checkIn` first (camelCase, from `toCamel` API normalization), then falls through `check_in → check_in_date → checkInDate` snake/camel variants; `formatClock` extended to handle plain `HH:mm` strings (for `reservationTime`/`entryTime`); meal block adds `Reservation · {formattedReservation}` (`data-testid="meal-reservation-fact"`) when `reservationTime` is present; activity block adds `Entry · {formattedEntryTime}` (`data-testid="activity-entry-fact"`) when `entryTime` is present; `hasAny` guard updated to include the new fields; inline meta-editor (`data-testid="itinerary-item-meta-editor"`) opened via "Edit stay dates / Edit reservation / Edit entry time" in the desktop overflow menu and mobile tray — hotels get two `type="date"` inputs (`itinerary-meta-check-in`, `itinerary-meta-check-out`), meals get one `type="time"` (`itinerary-meta-reservation-time`), activities get one `type="time"` (`itinerary-meta-entry-time`); save button (`itinerary-meta-save`).
- `api.ts`: new `updateItemMetadata(itemId, currentDetails, patch)` after `updateItemTimeline` — spreads `currentDetails`, merges/deletes specific keys (`checkIn/checkOut/reservationTime/entryTime`), calls `updateItem` (routes through `toSnake` on write). Empty string → `undefined` clears the key; existing snake_case variants removed on clear.
- `package.json`: `tests/stay-span-metadata-contract-v1.test.mjs` added to the explicit test list.
- `tests/itinerary-card-finalized-display.test.mjs`: updated `indexOf('item.itemType === "meal"')` to `indexOf('item.itemType === "meal" && (() => {')` and same for `activity` — discriminates the JSX render blocks from the `handleOpenMetaEditor` handler code introduced in this PR.

**Contracts preserved:** `addHotelToTrip` safely skips check-in/out date copy (ResearchResult.metadata has no typed date fields). `dayPart`/`timeLabel` fields not reused for the new fact displays. Brief = read-only. IdeasTray IA = placement-first (unchanged). TripBuilder = no `moveIdeaToTripIdeas` call for this path.

**Tests:** 3603 total; 3590 pass; 13 PRE-EXISTING failures (identical to baseline — verified).

### Itinerary card action normalization + fixed Move to Ideas path (PR #486)
...
### Journey Desk — Ideas tab polish (merged, PR #482)
...
- **Tests:** no test asserted the old label strings for these routes. 2 targeted suites run: 57/57 + 46/46 pass. 0 new failures.

### Home mobile IA cleanup — hide "Rooms in this house" on mobile (merged #479)
...
- **Premium hybrid action row** (`MapFoldOut.tsx` `PlannedItemCard`, Trip + Day lenses): `buildPlaced` returns `cards` alongside `pins` (same `extractItineraryCoordinates` gate); each card shows real fields only (title, day, real time, kind, saved note, rating/location, **Map**). Row = **Map** + **Move** icon chips (lucide `Map`/`CalendarDays`) and a **More** kebab (`MoreHorizontal`); Move only when `otherDays.length>0` (picker excludes the current day, no slot label). The **More** overflow menu holds the consequential actions (never side by side in the row): place-like items get **Back to Ideas** + **Remove from trip…**; flight/hotel/logistics anchors get **Manage in Itinerary** + **Remove from trip…** (`isAnchor = itemType !== meal && !== activity`).
- **Back to Ideas — non-destructive unplace (BLOCKER FIX).** Root cause found in preview: the backend `/ideas` list (`list_unscheduled_items`) only returns unscheduled items whose `details.source_kind ∈ {concierge_idea, saved_item}`. A placed item added directly from Explore has no curated `source_kind`, so nulling `day_id` alone left it **orphaned** (gone from the day *and* invisible in Trip Ideas — looked like a delete). Fix: new `api.unplaceItemToIdeas(itemId, currentDetails)` PATCHes `day_id:null` **and** stamps `source_kind:"concierge_idea"` when not already curated, **routed through `updateItem`** so the read-side camelCased `details` blob is re-snaked on write (the same path `updateIdeaMeta` uses). All other details (note/coordinates/rating/maps URL/reason) preserved; **never calls `deleteItem`**. The item now reliably reappears in the Ideas Lens / Ideas Tray. (`moveIdeaToTripIdeas` remains for TripBuilder, which has the same latent gap — a separate follow-up.)
- **Destructive safety:** **Remove from trip** (planned) and **Remove idea** (Ideas Lens) are the *only* `deleteItem` paths, each guarded by a two-step inline confirm (`map-planned-remove-confirm` / `map-idea-remove-confirm`) in the restrained `text-ds-warning` tone (no bright red). Copy is honest: **Back to Ideas** unplaces (never "remove" wording); only "Remove from trip"/"Remove idea" delete. **No "remove from map"/hide wording.**
...
- **Clarity:** Day-Lens empty state offers **Add from Ideas** only when ideas exist; the Ideas-Lens **Needs location** list states those ideas "can open in Google Maps but don't have coordinates to plot yet" — no geocoding/auto-placement suggestion.
- **Page wiring:** `handleItemUnplace(itemId, currentDetails)` (`unplaceItemToIdeas` + `refreshDaysAndIdeas`) as `onUnplace`; `handleIdeaRemove` also refreshes days; `onManageItinerary` opens the legacy Itinerary tab for anchors; Move-to-day reuses `onAssign`. After unplace, days + Trip Ideas re-fetch so the item leaves Trip/Day Lens and appears in Ideas Lens; Brief/Dayboard/Expanded Day stay coherent. v1A lenses + add-to-day unchanged; MapTiler config, Explore/Build map, and the Ideas Tray untouched.
- **Tests:** `tests/journey-desk-visual-itinerary-map-v1b.test.mjs` (hybrid Map/Move/More row; overflow menu holds unplace+remove; **Back to Ideas uses `unplaceItemToIdeas` via `updateItem`, stamps curated `source_kind`, never `deleteItem`**; only Remove-from-trip/Remove-idea call `deleteItem` after confirm; anchors→Manage in Itinerary; details preserved; selection sync; needs-location copy; no geocode/fake-coord scan). **Targeted map suite green (106/106); full suite identical 16 PRE-EXISTING failures vs current origin/main — none introduced.** tsc/next build via CI `certify` (node_modules absent here; screenshots via Vercel preview).
...
- **Out of scope:** **Map Fold-Out / lenses / numbered pins (v2)**; full desktop three-zone; day-part classifier consolidation. Legacy tabs retained (not removed). No AI organize, route optimization, drag/drop, providers, or SQL.
...
- **Out of scope (sequenced):** v1D desktop three-zone. **Map Fold-Out / lenses / numbered pins remain v2.** No AI organize, route optimization, drag/drop, or slot-level placement. (The legacy Ideas tab stays canonical idea management — see the parity plan; it is not folded/removed.)
...
### Next step
**Slice 1 is the current branch (open PR).** Slice 2 = ExpandedDay read-only travel hints (reuse `lib/travelHints`, omit when no location). Do not remove/hide/rename/demote legacy Itinerary or Ideas tabs until all High/Medium parity rows are covered (see `docs/ai/design/JOURNEY_DESK_ITINERARY_PARITY_PLAN.md`). Do not overload IdeasTray; do not duplicate functionality into Brief. Deferred: full desktop three-zone, consolidate day-part classifier. Visual Itinerary Map v1C waits for durable preference/status contract. Placement stays day-level (`assignIdeaToDay`); no slot-level persistence.
...
- 2026-05-25 — **PR open (current branch)** — Stay-span + reservation metadata contract v1: canonical `checkIn/checkOut` key priority, `Reservation·` / `Entry·` fact display, inline metadata editor, `updateItemMetadata` API helper. 55 new tests. 3603 total, 3590 pass, 13 PRE-EXISTING failures.
- 2026-05-25 — **PR #486 MERGED** — Itinerary card action normalization: `onUnplace` prop, overflow menu, `moveIdeaToTripIdeas` removed from this path, orphan-items root cause fixed. 17 new tests. 3548 total, 3535 pass, 13 PRE-EXISTING failures.
