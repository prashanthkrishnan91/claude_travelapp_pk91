...
- 2026-05-15 — **AI Concierge Evidence/Notes Credit ROI Reliability v1 (Level 2 backend).** New `backend/app/concierge/evidence_cache.py`: `build_evidence_fingerprint()` (MD5 over destination + subtype_concepts + location_modifiers + geography_hints + soft_prefs + version_salt), `should_run_editorial()` selectivity gate (skips Tavily for simple category searches), `EvidenceAtomCache` (TTL=3600s, in-memory, thread-safe), `NoteCache` (TTL=7200s, keyed by (place_id, evidence_fingerprint)), `CreditROITelemetry` (14 log fields, emitted per search turn). `set_level_writer.py`: added `make_cached_note_result()` — LLM-free note hydration from cache; `fallback_note_visible_count` invariant always 0. `semantic_retrieval.py`: Step 5.56 (editorial enrichment) is now cache-aware (evidence cache hit skips Tavily; accepted atoms cached even when LLM notes fail/time out); Step 5.8 (set-level writer) is now cache-aware (per-place note cache check; note cache write after quality gate; `credits_spent_but_no_visible_notes` credit waste signal). No SQL. No new providers. No visible behavior change. 57 new tests, all pass; 95 existing concierge tests pass.
