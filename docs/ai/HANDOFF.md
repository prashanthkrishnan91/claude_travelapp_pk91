...
**`ConciergePage.tsx` + `globals.css` — the /concierge scene now matches the approved concept** (`docs/ai/concepts/concierge-salon-concept-v1.html`, blueprint `docs/ai/design/CONCIERGE_SALON_PRODUCTION_BLUEPRINT.md`):
- `.atelier-salon-stage-panel` turns the main panel into a warm cinematic room (not a flat slab): deep base + two-point lamp warmth + brass hairline frame, still height-bounded with `overflow: hidden` so only the result canvas scrolls.
- `.atelier-salon-portal` is the emotional centerpiece — layered `scene → haze → bloom → grain → vignette` spans. The scene paints `var(--world-scenery)`, overridden per the typed destination via `pickWorldFromDestination(destination)` so the window **tunes toward the place** through the real world-DNA pipeline (no hardcoded mood logic, no fake destinations).
- Header + invitations live **inside** `.atelier-salon-portal-copy`; copy uses light cinema tokens (`--ds-pearl-cream`/`--ds-cream`) over a guaranteed dark scrim, so it can never go dark-on-dark regardless of the tuned scene.
- `data-salon-mode` ("open" pre-search → portal grows, canvas collapsed and non-scrolling; "tuned" post-search → portal collapses to a banner, canvas becomes the only scroller). The composer carries `atelier-salon-desk` and stays the fused desk at the stage base.
